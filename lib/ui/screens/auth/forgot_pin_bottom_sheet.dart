...
import 'package:base_mobile_app/ui/screens/auth/set_pin_screen.dart';
...
class _ForgotPinModelState extends State<ForgotPinModel> {
  ...
  bool get otpVisible{
    return (generateOtpResponse?.existingUser??false) && generateOtpResponse?.phoneNumber == "+91${_phoneNumberController.text}";
}
  ...
  Widget build(BuildContext context) {
    ...
    return GestureDetector(
      ...
      child: Container(
        padding: EdgeInsets.only(left: 20,right: 20,top: 20,bottom: 20+mediaQuery.viewInsets.bottom),
        child: Column(
            ...
            Column(
              ...
              children: [
                ...
                Column(
                  crossAxisAlignment: CrossAxisAlignment.start,
                  children: [
                    Text('forgot_pin.phone_number',style: theme.textTheme.bodySmall?.copyWith(color: ThemeColors.gray4),).tr(),
                    ...
                    Row(
                      children: [
                        ...
                        },icon: const Icon(Icons.edit_square,size: 18,color: Colors.black,),)
                      ],
                    ),
                    if(otpVisible)
            ...
            Visibility(
                ...
                replacement: ValueListenableBuilder(
                    ...
                    builder: (context, value, _) {
                      return ElevatedButton(onPressed: value.text.length == 10 ? _onSubmitClick : null, child: const Text('send_otp').tr());
                    }),
                child: ValueListenableBuilder(
                    valueListenable: _otpController,
                    builder: (context, value, _) {
                      return ElevatedButton(onPressed: value.text.length == 4 ? _onValidateOTP : null, child: const Text('submit').tr());
                    }))
          ],
        ),
      ),
